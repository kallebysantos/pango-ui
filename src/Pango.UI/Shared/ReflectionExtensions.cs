...
public static class ReflectionExtensions
{
    ...
    public static string ToNameString(this Type type, Func<Type, string> typeNameConverter = null)
    {
        return type.ToNameString(
            null,
            typeNameConverter == null
                ? (Func<Type, Queue<string>, string>)null
                : (t, _) => typeNameConverter(t)
        );
    }
    ...
    )
    {
        ...
        return "("
            + string.Join(
                ", ",
                parameters.Select(p =>
                    p.ToTypeNameString(typeNameConverter, invokeTypeNameConverterForGenericType)
                    + " "
                    + p.Name
                )
            )
    ...
    )
    {
        if (parameterInfo.ParameterType.IsByRef)
        {
            return (parameterInfo.IsIn ? "in " : (parameterInfo.IsOut ? "out " : "ref "))
                + parameterInfo
                    ...
                    .ToNameStringWithValueTupleNames(
                        parameterInfo
        ...
        return parameterInfo.ParameterType.ToNameStringWithValueTupleNames(
            parameterInfo.GetCustomAttribute<TupleElementNamesAttribute>()?.TransformNames,
            typeNameConverter,
            invokeTypeNameConverterForGenericType
        );
    }
    ...
    )
    {
        return fieldInfo.FieldType.ToNameStringWithValueTupleNames(
            fieldInfo.GetCustomAttribute<TupleElementNamesAttribute>()?.TransformNames,
            typeNameConverter,
            invokeTypeNameConverterForGenericType
        );
    }
    ...
    )
    {
        ...
        var decoratedTypeName = type.IsGenericType
            ? null
            : typeNameConverter?.Invoke(type, tupleFieldNames);
        ...
        else if (type.IsGenericType)
        {
            ...
            if (GenericTuples.Contains(genericTypeDefinition))
            {
                ...
                var tupleFields = type.GetGenericArguments()
                    .Select((arg) => (argumentType: arg, argumentName: tupleFieldNames?.Dequeue()))
                    .ToList();
                newTypeName =
                    "("
                    + string.Join(
                        ", ",
                        tupleFields.Select(arg =>
                            arg.argumentType.ToNameString(
                                tupleFieldNames,
                                typeNameConverter,
                                invokeTypeNameConverterForGenericType
            ...
            else if (type.Name.Contains('`'))
            {
                var genericTypeName = invokeTypeNameConverterForGenericType
                    ? typeNameConverter?.Invoke(genericTypeDefinition, tupleFieldNames)
                    : null;
                newTypeName =
    ...
    /// </summary>
    private static readonly HashSet<Type> GenericTuples = new(
        new Type[]
        {
            typeof(ValueTuple<>),
            typeof(ValueTuple<,>),
