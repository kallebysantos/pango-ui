...
public partial class ApiReference
{
    ...
    protected override void OnParametersSet()
    {
        _displayName = Component
            .Name.Replace("`1", $"<{GenericLabel}>")
            .Replace("`2", $"<{GenericLabel}>");
        _id = _displayName.Replace(' ', '-').ToLowerInvariant();
    }

    private string? Namespace => Component?.Namespace;
    ...
    [SuppressMessage(
        ...
        Justification = "Just for demo/documentation purposes"
    )]
    private IEnumerable<MemberDescription> GetMembers(MemberTypes type)
    {
        var MEMBERS_TO_EXCLUDE = new[]
        {
            "Id",
        ...
        if (_allMembers == null)
        {
                    ...
                    if (Component.IsGenericType)
                    {
                        if (InstanceTypes is null)
                        {
                            throw new ArgumentNullException(
                                nameof(InstanceTypes),
                                "InstanceTypes must be specified when Component is a generic type"
                            );
                        }
            ...
            var allMethods = Component
                .GetMethods()
                .Where(i => !i.IsSpecialName)
                .Select(i => (MemberInfo)i);
            ...
            foreach (var memberInfo in allProperties.Union(allMethods).OrderBy(m => m.Name))
            {
                try
                {
                    ...
                    // || Component.Name == "FluentComponentBase"
                    )
                    {
                        ...
                        if (propertyInfo != null)
                        {
                            ...
                            var isEvent =
                                ...
                                || (
                                    t.IsGenericType
                                    && t.GetGenericTypeDefinition() == typeof(EventCallback<>)
                                );
                            ...
                            if (isEvent)
                            {
                                var eventTypes = string.Join(
                                    ", ",
                                    propertyInfo.PropertyType.GenericTypeArguments.Select(i =>
                                        i.Name
                                    )
                                );
                                members.Add(
                        ...
                        if (methodInfo != null)
                        {
                            ...
                            if (methodInfo.IsGenericMethod)
                            {
                                genericArguments =
                                    ...
                                    + string.Join(
                                        ", ",
                                        methodInfo.GetGenericArguments().Select(i => i.Name)
                                    )
                                    + ">";
                            }
                            ...
                            members.Add(
                                new MemberDescription()
                                {
                                    ...
                                    [
                                        .. methodInfo
                                            .GetParameters()
                                            .Select(i => $"{i.ToTypeNameString()} {i.Name}"),
                                    ],
                ...
                {
                    Console.WriteLine(
                        $"[ApiDocumentation] ERROR: Cannot found {Component.FullName} -> {memberInfo.Name}"
                    );
                    throw;
                }
            }
    ...
    private static string GetDescription<T>(Type component, T memberInfo)
        where T : MemberInfo
    {
    ...
    private static string GetDescription(Type component, MethodInfo methodInfo)
    {
        ...
        var mangledName =
            methodInfo.Name + (genericArgumentCount == 0 ? "" : $"``{genericArgumentCount}");
