...
public static class StringExtensions
{
    ...
    public static string JoinMerge(this IEnumerable<string> values, char separator)
        => values.Aggregate<string, (StringBuilder, string)>(
                seed: (new StringBuilder(), string.Empty),
                func: (state, fragment) => state.Item2 != fragment
                    ? (state.Item1.AppendWithSeparator(separator, fragment), fragment)
                    : state
            ).Item1.ToString();

    public static StringBuilder AppendWithSeparator(this StringBuilder builder, char separator, string value)
    {
